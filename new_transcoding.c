...
static int open_input_file(const char *filename) 
{
    ...
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "Can not open input");
        return ret;
    }

    ret = avformat_find_stream_info(ifmt_ctx, NULL);
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "Cannot find strean information");
        return ret;
    }
...
static int open_output_file(const char *filename) 
{
    ...
    for (i = 0; i < ifmt_ctx->nb_streams; i++) {
        ...
        if (dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO || dec_ctx->codec_type == AVMEDIA_TYPE_AUDIO) {
            ...
            if (ret < 0) {
                av_log(NULL, AV_LOG_ERROR, "Can't open video encoder for stream #%u\n", i);
                return ret;
            }
            ...
            av_log(NULL, AV_LOG_FATAL, "Elementary stream #%d is of unknown type, can't proceed\n", i);
            return AVERROR_INVALIDDATA;
        } else {
            ret = avcodec_parameters_copy(out_stream->codecpar, in_stream->codecpar);
            if (ret < 0) {
                av_log(NULL, AV_LOG_ERROR, "Copying paramters from stream #%u failed\n", i);
                return ret;
            }
...
static int init_filter(FilteringContext *ftcx, AVCodecContext *dec_ctx, AVCodecContext *enc_ctx, const char *filter_spec) 
{
    char args[512];
    int ret = 0;
    ...
    if (dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO) {
        ...
        ret = avfilter_graph_create_filter(&buffersink_ctx, buffersink, "out", NULL, NULL, filter_graph);
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR, "Cannot create audio buffersink \n");
            goto end;
        }
    ...
    ftcx->buffersrc_ctx = buffersrc_ctx;
    ftcx->buffersink_ctx = buffersink_ctx;
    ftcx->filter_graph = filter_graph;
    ...
    while (1) {
        ...
        if (filter_ctx[stream_index].filter_graph) {
            ...
            while (1) {
                ...
                if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
                    av_log(NULL, AV_LOG_INFO, "Can't receive more frame\n");
                    av_frame_free(&frame);
                    break;
                }
